...
export function generateStaticParams() {
  return getPosts("lab-notes").map((p) => ({ slug: p.slug }));
}

export function generateMetadata({ params }: { params: { slug: string } }): Metadata {
  const post = getPost("lab-notes", params.slug);
  if (!post) return {};
  return {
...
export default function LabNote({ params }: { params: { slug: string } }) {
  const post = getPost("lab-notes", params.slug);
  if (!post) notFound();
  return (
    <main className="ruled-paper min-h-screen">
      ...
      <article className="mx-auto max-w-3xl px-4 py-14 sm:px-6">
        <p className="font-mono text-xs text-graphite/70">
          ...
          {post.meta.status ? ` · ${String(post.meta.status)}` : ""}
        </p>
